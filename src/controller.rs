...
use crate::config::{PREVIEW_MAX_WIDTH, PluginConfig};
use crate::media::{
    ConvertedVideo, PixelFormatKind, SessionClock, apply_alpha_policy, downscale_rgba, planar_f32,
    samples_per_frame,
};
use crate::ndi::{AudioJob, DropSender, LatestSlot, SendSession, VideoJob, drop_channel};
...
pub struct SharedState {
    ...
    pub audio_queued: Arc<AtomicUsize>,
...
impl SharedState {
    pub fn new() -> Self {
        Self {
            ...
            audio_queued: Arc::new(AtomicUsize::new(0)),
    ...
    fn start_output(&self) -> Result<(), String> {
        ...
        *self.shared.clock.lock().unwrap_or_else(|e| e.into_inner()) = SessionClock::new();
        ...
        let (audio_tx, audio_rx) = drop_channel::<AudioJob>(
            ...
            Arc::clone(&self.shared.audio_queued),
        );
        *self
            .shared
            .audio_tx
        ...
        *self.session.lock().unwrap_or_else(|e| e.into_inner()) = Some(session);
        ...
        self.shared.running.store(true, Ordering::Release);
        {
            ...
            status.tally_program = false;
            status.tally_preview = false;
            status.send_fps = 0.0;
            status.video_drops = 0;
            status.audio_drops = 0;
            status.queue_depth = 0;
            status.video_queued = 0;
...
fn push_output(shared: &SharedState, baked: &BakedScene, index: u32, live_audio: bool) {
    ...
    let Some(rgba) = baked.video.get(idx) else {
        return;
    };
    let timecode = shared.next_timecode();
    if config.send_video {
        let converted = apply_alpha_policy(
            ConvertedVideo {
                ...
                pixel_format: if baked.has_alpha {
...
fn update_preview(player: &CuePlayer, baked: &BakedScene, index: u32) {
    let Some(rgba) = baked.video.get(index as usize) else {
        return;
    };
    if let Some((width, height, preview)) =
        downscale_rgba(baked.width, baked.height, rgba, PREVIEW_MAX_WIDTH)
    {
